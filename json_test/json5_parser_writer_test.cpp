...
struct Test_runner {
    ...
    String_type to_str(const char* c_str) { return ::to_str<String_type>(c_str); }
    ...
    void check_eq_single_line_arrays(const Value_type& value,
    ...
    void test_obj_with_three_members() {
        ...
        check_eq(obj,
                 "{\"name_1\":\"value_1\",\"name_2\":\"value_2\",\"name_3\":\"value_"
                 "3\"}");
    ...
    void test_obj_with_grandchild_obj() {
        Object_type child_1;
        add_c_str(child_1, "name_1", "value_1");
        Object_type child_2;
        Object_type child_3;
        add_c_str(child_3, "name_3", "value_3");
        ...
        check_eq(root,
                 ...
                 "\"child_2\":{\"grandchild\":{\"name_3\":\"value_3\"},\"name_2\":"
                 "\"value_2\"},"
                 "\"name_a\":\"value_a\","
                 "\"name_b\":\"value_b\"}");
    ...
    void test_array_with_n_members() {
        ...
        check_eq_single_line_arrays(arr,
                                    "[ \"value_1\", 123, 123.456, true, false, null ]");
    }
    ...
    void test_array_with_grandchild_array() {
        Array_type child_1;
        child_1.push_back(11);
        Array_type child_2;
        child_2.push_back(22);
        Array_type child_3;
        child_3.push_back(33);
    ...
    void test_escape_char(const char* esc_str_in, const char* esc_str_out,
        ...
        const string out_str(string("{\"") + esc_str_out + "name\":\"value" +
                             esc_str_out + "\"}");
    ...
    void check_remove_trailing_zeros(const double value,
                                     const String_type& expected_str_with,
                                     const String_type& expected_str_without) {
        check_eq(value, expected_str_with, none);
        check_eq(value, expected_str_without, remove_trailing_zeros);
    }

    void check_remove_trailing_zeros(const double value, const char* expected_str_with,
                                     const char* expected_str_without) {
        check_remove_trailing_zeros(value, to_str(expected_str_with),
                                    to_str(expected_str_without));
    }
    ...
    void test_remove_trailing_zeros() {
        ...
        check_remove_trailing_zeros(1.2e-99, to_str("1.2e-") + exp,
                                    to_str("1.2e-") + exp);
        check_remove_trailing_zeros(1.23456789e99, to_str("1.2345678900000001e+") + exp,
                                    to_str("1.23456789e+") + exp);
    }

    void check_precision_of_doubles(unsigned int precision,
                                    const char* expected_result_1,
                                    const char* expected_result_2 = 0) {
        ...
        assert_eq(write(val, remove_trailing_zeros, precision),
                  to_str(expected_result_2));
    }

    void check_precision_of_doubles(const char* expected_result_1,
...
void test_extended_ascii() {
    const string expeced_result(is_printable('\xE4')
                                        ? "[\"\xE4\xF6\xFC\xDF\"]"
                                        : "[\"\\u00E4\\u00F6\\u00FC\\u00DF\"]");
