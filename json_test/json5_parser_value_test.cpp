...
void test_get_value() {
    ...
    Object obj;
    obj.push_back(Pair("name1", "value1"));

    test_get_value(obj);
}

void assert_array_eq(const Value& v, const Array& a) { assert_eq(v.get_array(), a); }

void assert_obj_eq(const Value& v, const Object& obj) { assert_eq(v.get_obj(), obj); }
...
template <typename T>
void check_wrong_type_exceptions(const Value& v, const string& requested_type_name,
                                 const string& actual_type_name) {
    try {
        v.get_value<T>();
        ...
        os << "get_value< " << requested_type_name << " > called on "
...
public:
    Variant_constructor_runner() {
        ...
        test_variant_constructor<variant<int, double, String_type> >(
                to_str<String_type>("foo"));
        test_variant_constructor<variant<int, double, String_type, bool> >(true);
        test_variant_constructor<
                variant<int, double, String_type, bool, boost::int64_t> >(
                boost::int64_t(123));
        test_variant_constructor<
                variant<int, double, String_type, bool, boost::uint64_t> >(
                boost::uint64_t(123));
